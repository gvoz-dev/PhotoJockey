...
public class XMLMenuHandler extends DefaultHandler {
    ...
    private final PJMainWindow app;
    private final Map<String, JComponent> menuStorage;
    private final Deque<JMenu> menus = new LinkedList<>();
    private JMenuBar menuBar;

    public XMLMenuHandler(PJMainWindow app, Map<String, JComponent> menuStorage) {
        this.app = app;
    ...
    private PJAction loadAction(String name) {
        ...
        if (name != null) {
            try {
                action = (PJAction) (Class.forName(name)).getConstructor(PJMainWindow.class).newInstance(app);
